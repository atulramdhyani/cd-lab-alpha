...
export default function App() {
  ...
  const FEEDBACK_FORM = "https://forms.gle/MhZAtR3t6soVqEGe8";
  const feedbackUI = <>
      ...
      <div style={{background:T.card,borderRadius:14,padding:24,maxWidth:400,width:"100%",border:`1px solid ${T.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.12)",animation:"fu 0.3s ease"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn v="accent" disabled={!fbText.trim()} onClick={()=>{
            ...
            navigator.clipboard.writeText(`${fbText}\n\n— ${email} | ${page} | ${new Date().toLocaleDateString()}`).catch(()=>{});
            window.open(FEEDBACK_FORM,"_blank");
            setShowFeedback(false);
          }} style={{flex:1}}>Send feedback</Btn>
          <Btn v="ghost" onClick={()=>setShowFeedback(false)}>Cancel</Btn>
        </div>
        <p style={{fontSize:11,color:T.muted,marginTop:10,lineHeight:1.4}}>Opens a short Google Form. Your feedback is also copied to clipboard for easy pasting.</p>
      </div>
    </div>}
  </>;
