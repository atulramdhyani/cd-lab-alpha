...
export default function App() {
  ...
  const newChallenge = () => { setView(V.BROWSE); setBrief(null); setAns({}); setExtras([]); setDone(false); setShowReview(false); setShowMockup(false); setBriefOpen(true); };
  ...
  const feedbackUI = <a href="https://forms.gle/MhZAtR3t6soVqEGe8" target="_blank" rel="noopener noreferrer" style={{position:"fixed",bottom:20,right:20,zIndex:200,padding:"10px 16px",borderRadius:10,background:T.text,color:T.bg,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'Outfit',sans-serif",boxShadow:"0 4px 16px rgba(0,0,0,0.15)",textDecoration:"none",transition:"transform 0.15s",display:"inline-block"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.05)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>💬 Feedback</a>;
    ...
    <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{maxWidth:420,width:"100%",animation:"fu 0.5s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <input placeholder="you@email.com" type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();const next=e.target.parentElement.querySelector('input[type=password]');if(next)next.focus();}}}
          ...
          <input placeholder="Alpha code" type="password" value={alphaCode} onChange={e=>{setAlphaCode(e.target.value);setGateError("");}} onKeyDown={e=>e.key==="Enter"&&gate()}
          ...
          <Btn disabled={!validEmail(email)||!alphaCode.trim()||view!==V.GATE} onClick={gate} style={{alignSelf:"flex-start"}}>Start →</Btn>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{maxWidth:800,margin:"0 auto",padding:"40px 22px"}}>
        {canGo ? (
          ...
          <div style={{textAlign:"center",padding:"40px 24px",background:T.errS,borderRadius:16,border:"1px solid rgba(184,58,58,0.12)",marginBottom:32}}>
            ...
            <p style={{color:T.sub,fontSize:14,marginBottom:4}}>Come back tomorrow for 5 more.</p>
            {hist.length > 0 && <p style={{color:T.sub,fontSize:13}}>In the meantime, revisit a past challenge below — try the self-review checklist or export a portfolio card.</p>}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{marginTop:10}}>
              {hist.slice(0,showAllHist?50:5).map(h => {
                ...
                return (
                  <button key={h.id} onClick={()=>{setBrief(h.brief);setAns(h.answers);setExtras(h.extras||[]);setDone(true);setShowReview(false);setChecks({});setCopied(false);setShowMockup(false);setBriefOpen(true);setView(V.CHALLENGE);window.scrollTo(0,0);}}
    ...
    return (
      <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:dev==="phone"?"1fr 370px":"1fr 1fr",maxWidth:1160,margin:"0 auto",minHeight:"calc(100vh - 240px)"}} className="wl-grid">
          {/* Form */}
          <div ref={formRef} style={{padding:"24px 24px 80px",borderRight:`1px solid ${T.border}`,overflowY:"auto"}} className={showMockup?"wl-form-hide":""}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <Mono>Write your copy</Mono>
            ...
            {(()=>{
              ...
              return <>
                {fields.some(f=>f.len>0) && <div style={{marginTop:20,padding:12,borderRadius:8,background:T.alt,border:`1px solid ${T.border}`}}>
                ...
                <div style={{marginTop:16,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                  {!done?<><Btn v="accent" disabled={!allGood} onClick={()=>{setDone(true);saveH(brief);setShowReview(true)}} title="⌘+Enter">✓ Mark Complete{allGood?"":" — "+( fields.length-filledCount)+" field"+(fields.length-filledCount===1?"":"s")+" left"}</Btn><span style={{fontSize:11,color:T.muted}}>⌘+Enter</span></>
            ...
            {done && (
              <div style={{marginTop:28,animation:"fu 0.4s ease"}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:12}}>
                  <Btn v="accent" style={{flex:1}} onClick={()=>exportScreenshot(brief,ans,extras,dark)}>
                    📸 Portfolio card
                  </Btn>
                  <Btn v="secondary" style={{flex:1}} onClick={()=>{const md=exportCaseStudy(brief,ans,extras);navigator.clipboard.writeText(md).then(()=>setCopied(true)).catch(()=>{});}}>
          ...
          <div style={{padding:24,display:"flex",alignItems:"flex-start",justifyContent:"center",background:T.bg,position:"sticky",top:130,alignSelf:"start"}} className={showMockup?"":"wl-mock-hide"}>
            ...
            <div style={{width:"100%"}}>
              ...
              {!done && Object.values(ans).every(v=>!v) && <p style={{textAlign:"center",fontSize:11,color:T.muted,marginBottom:10,marginTop:-8}}>Start typing — this preview updates live</p>}
