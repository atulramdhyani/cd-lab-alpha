...
function generateBrief(nicheId, toneId, screenId, personaId, NICHES, TONES, SCREENS, PERSONAS) {
  ...
  // Compose a self-contained problem statement
  const problem = `${ctx.detail}\n\nYour user is a ${persona.label} (~${persona.age}) — ${persona.d.toLowerCase()}. Write in a ${tone.label.toLowerCase()} tone (${tone.desc.toLowerCase()}). The copy needs to work for ${company.audience.toLowerCase()}.`;
  return { niche, tone, screen, persona, ctx, company, problem };
}
...
<div class="card">
  <div style="padding:28px 32px 20px;border-bottom:1px solid ${t.border}">
    ...
    <div style="padding:12px 14px;border-radius:8px;background:${t.aS};border:1px solid ${t.aB};font-size:13px;line-height:1.55;color:${t.sub}"><strong style="color:${t.text}">Problem Statement:</strong> ${esc(brief.problem).replace(/\n\n/g,'<br><br>')}</div>
  </div>
  ...
  if (view === V.CHALLENGE && brief) {
    ...
    return (
      <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh"}}>
        ...
        <div style={{background:T.alt,borderBottom:`1px solid ${T.border}`,padding:"20px 22px"}}>
          <div style={{maxWidth:920,margin:"0 auto"}}>
            ...
            {/* Problem statement — the main event */}
            <div style={{padding:16,borderRadius:10,background:T.card,border:`1.5px solid ${T.aB}`}}>
              <Mono>📋 Problem statement</Mono>
              <div style={{marginTop:8,fontSize:14,color:T.text,lineHeight:1.7}}>
                {brief.problem.split("\n\n").map((p,i) => <p key={i} style={{marginBottom:i===0?10:0}}>{p}</p>)}
              </div>
            </div>

            {/* What's on screen — collapsed */}
            <details style={{marginTop:8,borderRadius:8,background:T.card,border:`1px solid ${T.border}`,overflow:"hidden"}}>
              <summary style={{padding:"8px 14px",cursor:"pointer",fontSize:11,fontWeight:600,color:T.muted,letterSpacing:"0.04em",textTransform:"uppercase",fontFamily:"'IBM Plex Mono',monospace"}}>▸ What's on screen</summary>
              <div style={{padding:"4px 14px 12px",fontSize:13,color:T.sub,lineHeight:1.6}}>
                <p>{brief.ctx.visible}</p>
              </div>
            </details>
          </div>
        </div>
