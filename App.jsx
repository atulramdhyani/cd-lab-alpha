      ...
      <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh"}}>
        ...
        <div style={{background:T.alt,borderBottom:`1px solid ${T.border}`,padding:"18px 22px"}}>
          <div style={{maxWidth:920,margin:"0 auto"}}>
            ...
            {/* Task — the writing objective */}
            <div style={{padding:14,borderRadius:10,background:T.card,border:`1px solid ${T.border}`,marginBottom:10}}>
              <Mono>Your task</Mono>
              <p style={{fontSize:15,color:T.text,lineHeight:1.65,marginTop:6,fontWeight:500}}>{brief.screen.goal}</p>
              <p style={{fontSize:13,color:T.sub,lineHeight:1.55,marginTop:8}}>You're writing for <strong style={{color:T.text}}>{brief.company?.name}</strong> ({brief.company?.desc?.toLowerCase()}). Target audience: {brief.company?.audience?.toLowerCase()}. The user is a {brief.persona.label.toLowerCase()} (~{brief.persona.age}): {brief.persona.d.toLowerCase()}. Tone: {brief.tone.label.toLowerCase()} — {brief.tone.desc.toLowerCase()}.</p>
            </div>
