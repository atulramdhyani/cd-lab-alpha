...
export default function App() {
  ...
  // Keyboard shortcut: Cmd/Ctrl+Enter to mark complete
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === "Escape" && view === V.CHALLENGE) {
        setView(V.BROWSE);
      }
    };
  ...
  const gen = (n,t,s,p) => {
    ...
    setBrief(b); setAns({}); setExtras([]); setDone(false); setShowReview(false); setChecks({}); setCopied(false);
          ...
          <div>
            ...
            <div style={{marginTop:10}}>
              {hist.slice(0,8).map(h => {
                ...
                return (
                <button key={h.id} onClick={()=>{setBrief(h.brief);setAns(h.answers);setExtras(h.extras||[]);setDone(true);setShowReview(false);setChecks({});setCopied(false);setView(V.CHALLENGE)}}
  ...
  // ══════════════════════════════════════════════
  // CHALLENGE — brief, form, mockup, review, export
  // ══════════════════════════════════════════════
      ...
      <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh"}}>
        ...
        <div style={{background:T.alt,borderBottom:`1px solid ${T.border}`,padding:"18px 22px"}}>
          <div style={{maxWidth:920,margin:"0 auto"}}>
            ...
            {/* Goal — the writing objective, biggest and boldest */}
            ...
            {/* Scannable chips: persona, tone, audience */}
            ...
            <div style={{padding:12,borderRadius:10,background:"rgba(199,67,43,0.04)",border:`1px solid ${T.aB}`,marginBottom:10}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:6}}>
                {brief.screen.components.map(k => {
                  ...
                  return <span key={k} style={{fontSize:12,color:T.text,padding:"3px 9px",borderRadius:5,background:T.card,border:`1px solid ${T.border}`}}>
                    <strong>{m.label}</strong> <span style={{color:T.muted,fontSize:10}}>({m.max})</span>
                  </span>;
                })}
              </div>
            </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:dev==="phone"?"1fr 370px":"1fr 1fr",maxWidth:1160,margin:"0 auto",minHeight:"calc(100vh - 240px)"}}>
          ...
          <div style={{padding:"24px 24px 80px",borderRight:`1px solid ${T.border}`,overflowY:"auto"}}>
            ...
            {(()=>{
              const fields = brief.screen.components.map(k => {
                const m = COMP[k]; if (!m) return null;
                const val = (ans[k]||"").trim();
                const min = Math.ceil(m.max * 0.3);
                const over = val.length > m.max;
                const good = val.length >= min && !over;
                const empty = val.length === 0;
                const short = val.length > 0 && val.length < min;
                return { k, label: m.label, good, over, empty, short, len: val.length, max: m.max, min };
              }).filter(Boolean);
              const allGood = fields.every(f => f.good);
              const anyOver = fields.some(f => f.over);
              const filledCount = fields.filter(f => f.good).length;
              return <>
                {/* Per-field status strip */}
                <div style={{marginTop:20,padding:12,borderRadius:8,background:T.alt,border:`1px solid ${T.border}`}}>
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {fields.map(f => (
                      <span key={f.k} style={{fontSize:11,padding:"3px 8px",borderRadius:5,fontWeight:500,background:f.good?T.okS:f.over?T.errS:f.short?"#FEF3C7":T.card,color:f.good?T.ok:f.over?T.err:f.short?"#92400E":T.muted,border:`1px solid ${f.good?"rgba(45,122,79,0.15)":f.over?"rgba(184,58,58,0.15)":f.short?"rgba(146,64,14,0.15)":T.border}`}}>
                        {f.good?"✓":"○"} {f.label} <span style={{opacity:0.7}}>{f.len}/{f.max}</span>
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div style={{marginTop:16,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                  {!done ? (
                    <Btn v="accent" disabled={!allGood} onClick={()=>{setDone(true);saveH(brief);setShowReview(true)}}>
                      ✓ Mark Complete {allGood?"":`(${fields.length - filledCount} remaining)`}
                    </Btn>
                  ) : (
            ...
            {done && (
              <div style={{marginTop:28,animation:"fu 0.4s ease"}}>
                ...
                {/* Export */}
                <Btn v="secondary" style={{width:"100%",marginBottom:12}} onClick={() => {
                ...
                {/* New Challenge — demoted to ghost button after completion */}
                <Btn v="ghost" style={{width:"100%",textAlign:"center",justifyContent:"center"}} onClick={()=>{setView(V.BROWSE);setBrief(null);setAns({});setExtras([]);setDone(false);setShowReview(false)}}>
