...
function exportScreenshot(brief, ans, extras, isDark) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${esc(brief.company?.name)} — ${esc(brief.screen.label)} | Wordsmith Lab</title>
...
<div class="card">
  <div style="padding:28px 32px 20px;border-bottom:1px solid ${t.border}">
    ...
    <div style="padding:12px 14px;border-radius:8px;background:${t.aS};border:1px solid ${t.aB};font-size:13px;line-height:1.55;color:${t.sub}"><strong style="color:${t.text}">Objective:</strong> ${esc(brief.screen.goal)}</div>
  </div>
  ...
  <div style="padding:14px 32px;background:${t.alt};border-top:1px solid ${t.border};display:flex;justify-content:space-between;align-items:center">
    <span style="font-size:11px;color:${t.muted};font-weight:500">Wordsmith Lab — UX Writing Practice</span>
...
export default function App() {
  ...
  const ALPHA_PASS = "wordsmith-alpha"; // Change this to rotate access
  const gate = () => {
  ...
  const gen = (n,t,s,p) => {
    ...
    setTimeout(() => {
      ...
      setBrief(b); setAns({}); setExtras([]); setDone(false); setShowReview(false); setChecks({}); setCopied(false); setShowMockup(false);
  ...
  const FeedbackBtn = () => <>
    <button onClick={()=>{setShowFeedback(true);setFbSent(false);setFbText("");}} style={{position:"fixed",bottom:20,right:20,zIndex:200,padding:"10px 16px",borderRadius:10,background:T.text,color:T.bg,border:"none",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'Outfit',sans-serif",boxShadow:"0 4px 16px rgba(0,0,0,0.15)",transition:"transform 0.15s"}} onMouseEnter={e=>e.target.style.transform="scale(1.05)"} onMouseLeave={e=>e.target.style.transform="scale(1)"}>💬 Feedback</button>
    {showFeedback && <div style={{position:"fixed",inset:0,zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.4)",padding:20}} onClick={e=>{if(e.target===e.currentTarget)setShowFeedback(false)}}>
      <div style={{background:T.card,borderRadius:14,padding:24,maxWidth:400,width:"100%",border:`1px solid ${T.border}`,boxShadow:"0 8px 32px rgba(0,0,0,0.12)",animation:"fu 0.3s ease"}}>
        {fbSent ? <>
          <p style={{fontSize:18,fontWeight:600,marginBottom:6}}>Thanks for the feedback! 🙏</p>
          <p style={{fontSize:14,color:T.sub,marginBottom:16}}>Your feedback has been copied to your clipboard. Please paste it in an email or message to share it with us.</p>
          <Btn onClick={()=>setShowFeedback(false)} style={{width:"100%"}}>Close</Btn>
        </> : <>
          ...
          <div style={{display:"flex",gap:8}}>
            <Btn v="accent" disabled={!fbText.trim()} onClick={()=>{
              const fb = `[Wordsmith Lab Feedback]\nFrom: ${email}\nDate: ${new Date().toISOString()}\nPage: ${view===V.CHALLENGE&&brief?brief.company?.name+" — "+brief.screen.label:"Browse"}\n\n${fbText}`;
              navigator.clipboard.writeText(fb).then(()=>setFbSent(true)).catch(()=>setFbSent(true));
            }} style={{flex:1}}>Copy feedback</Btn>
    ...
    <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{maxWidth:420,width:"100%",animation:"fu 0.5s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <Btn disabled={!email.trim()||!email.includes("@")||!alphaCode.trim()} onClick={gate} style={{alignSelf:"flex-start"}}>Start →</Btn>
        </div>
      </div>
    </div>
  );
          ...
          <div style={{animation:"fu 0.3s ease",marginBottom:32}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:20}} className="wl-brief-grid">
              {[{l:"Product Niche",items:NICHES,sel:sN,set:setSN,r:n=>`${n.icon} ${n.label}`,dk:null},
                ...
                {l:"Screen Type",items:SCREENS,sel:sS,set:setSS,r:s=>`${s.icon} ${s.label}`,dk:null},
          ...
          <div>
            ...
            <div style={{marginTop:10}}>
              {hist.slice(0,8).map(h => {
                ...
                return (
                  <button key={h.id} onClick={()=>{setBrief(h.brief);setAns(h.answers);setExtras(h.extras||[]);setDone(true);setShowReview(false);setChecks({});setCopied(false);setShowMockup(false);setView(V.CHALLENGE)}}
      ...
      <div style={{fontFamily:"'Outfit',sans-serif",color:T.text,background:T.bg,minHeight:"100vh"}}>
        ...
        <div style={{borderBottom:`1px solid ${T.border}`,padding:"9px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",background:T.card,position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:6}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span title={`${used} of ${LIM} challenges used today`} style={{cursor:"help"}}><Tag>{used}/{LIM}</Tag></span>
          </div>
        </div>
        ...
        <div style={{background:T.alt,borderBottom:`1px solid ${T.border}`,padding:"18px 22px"}}>
          <div style={{maxWidth:920,margin:"0 auto"}}>
            ...
            {/* Goal */}
            <div style={{padding:16,borderRadius:10,background:T.card,border:`1.5px solid ${T.aB}`,marginBottom:12}}>
              <p style={{fontSize:15,color:T.text,lineHeight:1.65,fontWeight:500}}>{brief.screen.goal}</p>
            </div>
            {/* Persona / Tone / Audience / Niche grid */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}} className="wl-brief-grid">
              ...
              <div style={{padding:10,borderRadius:8,background:T.card,border:`1px solid ${T.border}`}}>
                <Mono>🎯 Audience</Mono>
                <p style={{fontSize:13,color:T.text,marginTop:4}}>{brief.company?.audience}</p>
              </div>
              <div style={{padding:10,borderRadius:8,background:T.card,border:`1px solid ${T.border}`}}>
                <Mono>{brief.niche.icon} {brief.niche.label}</Mono>
                <p style={{fontSize:13,color:T.text,marginTop:4}}>{brief.screen.icon} {brief.screen.label}</p>
              </div>
            </div>
            {/* Deliverables */}
            <div style={{padding:12,borderRadius:10,background:T.aS,border:`1px solid ${T.aB}`,marginBottom:10}}>
              <Mono>✍️ Write these {brief.screen.components.length} components</Mono>
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:6}}>
                {brief.screen.components.map(k => { const m=COMP[k]; if(!m)return null; return <span key={k} style={{fontSize:12,color:T.text,padding:"3px 9px",borderRadius:5,background:T.card,border:`1px solid ${T.border}`}}><strong>{m.label}</strong> <span style={{color:T.muted,fontSize:10}}>({m.max})</span></span>; })}
              </div>
            </div>
            {/* Context */}
            <details style={{borderRadius:10,background:T.card,border:`1px solid ${T.border}`,overflow:"hidden"}}>
              <summary style={{padding:"10px 14px",cursor:"pointer",fontSize:12,fontWeight:600,color:T.sub,letterSpacing:"0.04em",textTransform:"uppercase",fontFamily:"'IBM Plex Mono',monospace"}}>▸ Context — situation details</summary>
              <div style={{padding:"4px 14px 14px",fontSize:13,color:T.sub,lineHeight:1.7}}>
          ...
          <div style={{padding:"24px 24px 80px",borderRight:`1px solid ${T.border}`,overflowY:"auto"}} className={showMockup?"wl-form-hide":""}>
            ...
            {(()=>{
              ...
              return <>
                ...
                <div style={{marginTop:16,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                  {!done?<Btn v="accent" disabled={!allGood} onClick={()=>{setDone(true);saveH(brief);setShowReview(true)}} title="⌘+Enter">✓ Mark Complete{allGood?"":" ("+( fields.length-filledCount)+" remaining)"}</Btn>
            ...
            {done && (
              <div style={{marginTop:28,animation:"fu 0.4s ease"}}>
                ...
                <Btn v="ghost" style={{width:"100%",textAlign:"center",justifyContent:"center"}} onClick={newChallenge}>Generate another challenge →</Btn>
              </div>
            )}
          </div>
